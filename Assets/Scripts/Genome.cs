...
public class Genome
{
    ...
    public float[] starting = new float[2]; //determines the starting position of the ai - 0 index is angle, 1 index is position
    ...
    public Genome(int id, int inputs, int output, Innovation innovation)
    {
        ...
        starting[0] = Random.Range(45, 135f);
        starting[1] = Random.Range(-9f, 9f);
...
public void MutateStarting(double mut_rate)
    {
        if(Random.Range(0f,1f) < mut_rate)
        {
            starting[0] += Random.Range(-10f, 10f); //angle
            starting[1] += Random.Range(-1.5f, 1.5f); //starting pos

            //clamp values
            starting[0] = Clamp(starting[0], 1f, 179f);
            starting[1] = Clamp(starting[1], -9f, 9f);
